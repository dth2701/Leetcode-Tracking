...
def organize_pirate_crew(group_sizes):
    # Group pirates by their required group size
    # key: size , value: a list of pirates
    dict = {}
    for pirate, size in enumerate(group_sizes):
        if size not in dict: 
            dict[size] = []
        dict[size].append(pirate)
    print(dict)
    # Mission: return a list of groups
    # Tranverse all items
    # Handle the oversize group by slicing into many groups with same size
    # Finally, add all groups in the big list to return
    result = []
    # O(n)
    for size, pirates in dict.items():
